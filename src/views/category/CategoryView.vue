...
type ResourceType = Prisma.CategoryGetPayload<{
  include: {
    categoryOnMedia: {
      select: {
        media: {
...
const getCategory = async () => {
  ...
  const res = await requestInstance.get<ResponseSuccess<ResourceType>>(`/category/${categoryId}`, {
    params: {
      page: 1,
      take: 9999
    }
  ...
  dataSource.value = res.data.data.categoryOnMedia.map(item => {
    return {
      media: item.media,
...
const handler = ref(getCategory())

watch(() => route.params.albumId, () => {
  handler.value = getCategory()
})
