...
export const playMedia = (media: Prisma.MediaGetPayload<{
  include: {
    owner: true;
    thumbnails: true;
  }
...
export const calculateDurationBetweenTwoDays = (start: Date, end: Date): string => {
  const duration = dayjs.duration(new Date(start).getTime() - new Date(end).getTime())
  if (duration.years() > 1) {
